...
const CandidateOverview = async () => {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/appliedJobs/2?limit=3`);
            ...
            <table className="w-full border-collapse min-w-[600px] md:min-w-full">
              ...
              <thead>
                <tr className="bg-gray-100 text-gray-600 uppercase text-xs md:text-sm leading-normal">
                  ...
                  <th className="py-2 px-3 md:py-3 md:px-6 text-left">Location</th>
                  <th className="py-2 px-3 md:py-3 md:px-6 text-left">Salary</th>
                  <th className="py-2 px-3 md:py-3 md:px-6 text-left">Date Applied</th>
              ...
              <tbody className="text-gray-700 text-xs md:text-sm">
                {data.map((d: JobAppType) => (
                  <tr key={d.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3 md:py-3 md:px-6">{d.appliedAt ? new Date(d.appliedAt).toLocaleDateString() : ''}</td>
                    <td className={`py-2 px-3 md:py-3 md:px-6 ${d.status === 'rejected' ? 'text-red-500' : 'text-green-500'} font-medium`}>
                      {d.status}
                    </td>
                    <td className="py-2 px-3 md:py-3 md:px-6 text-right">
                      <Link href={`/jobs/${d.job.id}`} className="bg-blue-500 text-white px-3 py-1 md:px-4 md:py-2 rounded-md text-xs md:text-sm">
                        View Details
                      </Link>
                    </td>
                  </tr>
    ...
    return (
      <div>error return candidate aplied jobs</div>
    )
  }
}
