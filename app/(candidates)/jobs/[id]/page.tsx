...
const JobDetails = async ({ params }: { params: Promise<{ id: string }> }) => {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/jobs/${resolvedParams.id}`);
      ...
      {/* Job Header */}
        <div className="rounded-lg flex flex-col md:flex-row gap-6">
          <div className="w-full flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center md:items-start">
              <div className="w-16 h-16 bg-gray-200 rounded-full">{data.company.profilePicture}</div>
           ...
           `       <p className="text-gray-600 text-sm">at {data.company.name}</p>
                  <div className="flex items-center gap-2">
                    <span className="bg-green-100 text-green-700 text-xs font-medium px-2 py-1 rounded">{data.jobType}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center border-b pb-4">
          <div className="text-center md:text-left">
            ...
            <p className="text-green-600 text-lg font-semibold">${Math.floor(Number(data.salary.split('$')[1]))}</p>
          </div>
        ...
        <div className="py-4 border-t">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="flex flex-col">
              ...
              <p className="font-semibold text-black text-sm">{new Date(data.createdAt).toLocaleDateString()}</p>
            </div>
            <div className="flex flex-col">
              <FaRegClock className="mr-2 text-primary500 mb-2 text-xl" />
              <p className="text-gray-600">
                Job Expire:</p>
                <p className="font-semibold text-black text-sm">{new Date(new Date(data.createdAt).setMonth(new Date(data.createdAt).getMonth() + 1)).toLocaleDateString()}</p>
            </div>
            <div className="flex flex-col">
              <FaBriefcase className="mr-2 text-primary500 mb-2 text-xl" />
              <p className="text-gray-600">Job Level</p>
              <p className="font-semibold text-black text-sm">{data.jobLevel}</p>
            </div>
            <div className="flex flex-col">
              <FaWallet className="mr-2 text-primary500 mb-2 text-xl" />
              <p className="text-gray-600">Experience:</p>
              <p className="font-semibold text-black text-sm">{data.experience} years</p>
            </div>
            <div className="flex flex-col">
              <FaGraduationCap className="mr-2 text-primary500 mb-2 text-xl" />
              <p className="text-gray-600">Education:</p>
              <p className="font-semibold text-black text-sm">{data.education}</p>
            </div>
          </div>
        </div>
